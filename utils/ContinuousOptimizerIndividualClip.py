...
class ContinuousOptimizerIndividualClip:
    def __init__(self, discrete_solution, server, tokenizer, device, client_grads,
                 ...
                 lr_decay_type, simreg=100, grad_clip=0.5,
    ...
    def optimize(self):
        ...
        for i in range(self.num_of_iterations):
            ...
            with torch.no_grad():
                grad_norm = self.dummy_embedding.grad.norm()
                if grad_norm > self.grad_clip:
                    self.dummy_embedding.grad.mul_(self.grad_clip / (grad_norm + 1e-6))
