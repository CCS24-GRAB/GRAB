...
class ContinuousOptimizerDropout():
    def __init__(self, discrete_solution, client, server, tokenizer, device, client_grads,
                 token_embedding, labels, alpha, beta, longest_length, individual_lengths, batch_size, seq_len, init_size, num_perm, lr,
    ...
    def construct_input_embeddings(self):
        if self.discrete_solution is not None:
            ...
            for i in range(len(perms)):
                ...
                input_embeddings = torch.stack(input_embeddings).to(self.device).requires_grad_(True)
                perms_embeddings.append(input_embeddings)

            for i in range(len(perms_embeddings)):
                if i % 100 == 0:
                    print(f"Evaluating discrete perm {i}")
                perm_embedding = perms_embeddings[i]
                ...
                valid_embedding = perm_embedding[:, 1:self.longest_length - 1, :].clone().detach()
                ...
                grads_diff += 100 * similarity_regularization
            ...
            for i in range(len(embeddings)):
                ...
                # Add the similarity regularization term
                valid_embedding = dummy_embedding[:, 1:self.longest_length - 1, :].clone().detach()
                valid_embedding_mean = valid_embedding.mean(dim=1)
                # Calculate pairwise cosine similarity between rows
                similarity_matrix = F.cosine_similarity(valid_embedding_mean.unsqueeze(1),
                                                        valid_embedding_mean.unsqueeze(0), dim=-1).to(self.device)
    ...
    def optimize(self):
        ...
        for i in range(self.num_of_iterations):
            ...
            # Add the similarity regularization term
            valid_embedding = self.dummy_embedding[:, 1:self.longest_length - 1, :].clone().detach()
            valid_embedding_mean = valid_embedding.mean(dim=1)
            # Calculate pairwise cosine similarity between rows
            similarity_matrix = F.cosine_similarity(valid_embedding_mean.unsqueeze(1),
                                                    valid_embedding_mean.unsqueeze(0), dim=-1).to(self.device)
