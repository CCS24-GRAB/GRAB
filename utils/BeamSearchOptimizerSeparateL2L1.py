from .GreedyOptimizerSeparateL2L1 import GreedyOptimizerSeparateL2L11
import torch
import joblib
import copy
import random
...
class BeamSearchOptimizerSeparateL2L1(GreedyOptimizerSeparateL2L11):
    def __init__(self, client, server, tokenizer, device, seq_len, batch_size, client_grads, result_file,
                 labels, start_tokens, longest_length, separate_tokens, token_set, alpha=0.05,
                 num_of_solutions=64, num_of_iter=5, num_of_perms=2000, continuous_solution=None, discrete_solution=None,beam=5):
        super().__init__(client, server, tokenizer, device, seq_len, batch_size, client_grads, result_file,
                         labels, start_tokens, longest_length, separate_tokens, token_set, alpha,
