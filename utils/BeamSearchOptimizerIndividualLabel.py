from .GreedyOptimizerIndividualLabel import GreedyOptimizerIndividualLabel
import copy
import numpy as np


class BeamSearchOptimizerIndividualLabel(GreedyOptimizerIndividualLabel):
    def __init__(self, server, tokenizer, device, seq_len, batch_size, client_grads, result_file,
        ...
        for tokens in self.separate_tokens:
            tks = [tk for tk in tokens if tk not in self.tokenizer.all_special_ids]
    ...
    def optimize_one_solution(self, index):
        ...
        for j in range(self.batch_size):
            ...
            for k in range(1, length - 1):
                temp_solutions = []
                temp_objs = []
                for token in [tk for tk in separate_tokens if tk not in self.tokenizer.all_special_ids]:
