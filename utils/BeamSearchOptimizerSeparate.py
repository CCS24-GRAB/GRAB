from .GreedyOptimizerSeparate import GreedyOptimizerSeparate
import torch
import joblib
import copy
import random
...
class BeamSearchOptimizerSeparate(GreedyOptimizerSeparate):
    def __init__(self, client, server, tokenizer, device, seq_len, batch_size, client_grads, token_set, result_file,
                 ...
                 num_of_solutions=64, num_of_iter=5, continuous_solution=None, discrete_solution=None,beam=5):
        super().__init__(client, server, tokenizer, device, seq_len, batch_size, client_grads, token_set, result_file,
                         labels, start_tokens, longest_length, separate_tokens,
                         num_of_solutions, num_of_iter, continuous_solution, discrete_solution)
        self.beam = beam
