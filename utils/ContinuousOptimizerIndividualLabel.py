...
class ContinuousOptimizerIndividualLabel:
    def __init__(self, discrete_solution, server, tokenizer, device, client_grads,
                 token_embedding, dummy_labels, alpha, beta, individual_lengths, batch_size, seq_len, init_size, num_perm, lr,
    ...
    def construct_input_embeddings(self):
        if self.discrete_solution is not None:
            ...
            for i in range(len(perms_embeddings)):
                ...
                perm_loss = loss_function(perm_preds, self.dummy_labels)
            ...
            for i in range(len(embeddings)):
                ...
                loss_function = torch.nn.CrossEntropyLoss()
                dummy_loss = loss_function(dummy_preds, self.dummy_labels)
                # self.server.zero_grad()
                server_parameters = []
                for param in self.server.parameters():
                    if param.requires_grad:
    ...
    def optimize(self):
        self.set_up()
        self.dummy_labels.requires_grad_(True)
        optimizing_tensors = []
        optimizing_tensors.append(self.dummy_embedding)
        optimizing_tensors.append(self.dummy_labels)
        ...
        for i in range(self.num_of_iterations):
            ...
            self.dummy_labels.data.clamp_(0, 1)
        ...
        #     param.detach_()
        self.server.zero_grad()
        return copy.deepcopy(self.server), -self.grads_diff, self.continuous_solution, self.dummy_labels.clone().detach()
        # with torch.no_grad():
