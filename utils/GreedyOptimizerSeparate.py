    ...
    def set_up(self):
        ...
        for i in range(self.num_of_solutions):
            ...
            for j in range(self.batch_size):
                ...
                start_tokens = [token for token in separate_tokens if token in self.start_tokens]
                if len(start_tokens) == 0:
                    start_tokens = separate_tokens
                non_special_tokens = [token for token in separate_tokens if token in self.non_special_token_set]
                if len(non_special_tokens) == 0:
                    non_special_tokens = separate_tokens
                sequence = [self.tokenizer.cls_token_id, random.choice(start_tokens)]
                for k in range(self.estimated_len - 3):
                    token = random.choice(non_special_tokens)
