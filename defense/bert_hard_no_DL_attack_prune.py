...
parser.add_argument('--prune', default=None, type=float, help="gradient pruning percentage")
...
prune = args.prune
RESULT_FILE = f"../results/defense/prune/{prune}/hard_no_DL/{args.dataset}_bert_attack_hard_simreg_{args.run}_run_{BATCH_SIZE}.txt"
...
for training_batch in train_loop:
    ...
    client.train()
    server.eval()
    loss, attentions, hidden_states, logits = train_batch_with_frozen_weights_and_prune(client, training_batch, DEVICE,
                                                                                        prune)
    ...
    for i in range(5):
        ...
        continuous_optimizer = ContinuousOptimizerIndividualPrune(copy.deepcopy(discrete_solution),
                                                                  ...
                                                                  seq_len=sequence_length, init_size=2000,
                                                                  num_perm=2000,
        ...
        continuous_obj = calculate_discrete_obj_prune(copy.deepcopy(continuous_solution), copy.deepcopy(labels),
                                                      copy.deepcopy(server), copy.deepcopy(client_grads), DEVICE,
                                                      tokenizer, prune_mask=prune_mask,
                                                      alpha=0.01)
        ...
        discrete_optimizer_separate = BeamSearchOptimizerIndividualPrune(copy.deepcopy(server), tokenizer, DEVICE,
                                                                         sequence_length,
                                                                         ...
                                                                         continuous_solution=copy.deepcopy(
                                                                             continuous_solution),
                                                                         discrete_solution=copy.deepcopy(
                                                                             discrete_solution),
                                                                         beam=4)
