...
noise = args.noise
RESULT_FILE = f"../results/defense/noise/{noise}/hard_no_DL/{args.dataset}_bert_attack_hard_simreg_faster_{args.run}_run_{BATCH_SIZE}.txt"
...
for training_batch in train_loop:
    ...
    client.train()
    server.eval()
    loss, attentions, hidden_states, logits = train_batch_with_frozen_weights_and_noise(client, training_batch, DEVICE, noise)
