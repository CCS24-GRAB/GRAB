...
parser.add_argument('--noise', default=None, type=float, help="defense noise level")
...
noise = args.noise
RESULT_FILE = f"../results/defense/noise/{noise}/{args.dataset}_bert_attack_hard_simreg_{args.run}_run_{BATCH_SIZE}.txt"
print("Adding noise: ", bool(noise))
...
for training_batch in train_loop:
    ...
    if noise is None:
        ...
        loss, attentions, hidden_states, logits = train_batch_with_frozen_weights_and_noise(client, training_batch,
                                                                                            DEVICE, noise)
    ...
    for i in range(5):
        ...
        continuous_optimizer = ContinuousOptimizerIndividualDP(copy.deepcopy(discrete_solution), copy.deepcopy(server),
                                                             tokenizer,
        ...
        continuous_obj = calculate_discrete_obj_DP(copy.deepcopy(continuous_solution), copy.deepcopy(labels),
        ...
        discrete_optimizer_separate = BeamSearchOptimizerIndividualDP(copy.deepcopy(server), tokenizer, DEVICE,
                                                                    sequence_length,
