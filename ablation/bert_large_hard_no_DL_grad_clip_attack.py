...
parser = ArgumentParser(description='bert_large_attack')
parser.add_argument('--device', default="cuda:0", type=str, help='cuda device')
parser.add_argument('--model', default="bert-large-uncased", type=str, help='large language model')
parser.add_argument('--dataset', default="cola", type=str, help='dataset for task')
parser.add_argument("--batch_size", default=1, type=int, help="batch size")
parser.add_argument("--lr_decay_type", default="LambdaLR", type=str, help="lr decay type")
...
RESULT_FILE = f"../results/ablation/models/large/hard_no_DL/grad_clip/{args.dataset}_bert_large_attack_hard_simreg_{args.run}_run_{BATCH_SIZE}.txt"
...
for training_batch in train_loop:
    ...
    for i in range(5):
        ...
        continuous_optimizer = ContinuousOptimizerIndividualClip(copy.deepcopy(discrete_solution), copy.deepcopy(server),
                                                             tokenizer,
