...
RESULT_FILE = f"../results/ablation/dropout/{args.dropout}/{args.dataset}_bert_attack_hard_simreg_faster_{args.run}_run_{BATCH_SIZE}.txt"
...
server.bert.embeddings.dropout.p = dropout_rate
j = 1
for i in range(0, 12):
    server.bert.encoder.layer[i].attention.self.dropout.p = dropout_rate
    j += 1
    server.bert.encoder.layer[i].attention.output.dropout.p = dropout_rate
    j += 1
    server.bert.encoder.layer[i].output.dropout.p = dropout_rate
    j += 1
server.dropout.p = dropout_rate
