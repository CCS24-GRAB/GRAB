...
RESULT_FILE = f"../results/ablation/label_longest_length/{args.dataset}_bert_attack_hard_simreg_faster_{args.run}_run_{BATCH_SIZE}.txt"
...
# # Turn off all bias in encoder blocks
# new_linear_layers = []
# for name, module in server.named_modules():
#     if "attention" in name and isinstance(module, torch.nn.Linear):
#         new_linear = torch.nn.Linear(module.in_features, module.out_features, bias=False)
#         new_linear.weight.data = module.weight.data.clone()
#         new_linear_layers.append(new_linear)
# # Replace the old linear layers with the new ones
# j = 0
# for i in range(12):
#     server.bert.encoder.layer[i].attention.self.query = new_linear_layers[j]
#     j += 1
#     server.bert.encoder.layer[i].attention.self.key = new_linear_layers[j]
#     j += 1
#     server.bert.encoder.layer[i].attention.self.value = new_linear_layers[j]
#     j += 1
#     server.bert.encoder.layer[i].attention.output.dense = new_linear_layers[j]
#     j += 1
# server.bert.encoder.layer[i].intermediate.dense = new_linear_layers[j]
# j += 1
# server.bert.encoder.layer[i].output.dense = new_linear_layers[j]
# j += 1
...
for training_batch in train_loop:
    ...
    for i in range(len(individual_lengths)):
        individual_lengths[i] = longest_length
    ...
    for i in range(5):
        ...
        continuous_optimizer = ContinuousOptimizerIndividualLabel(copy.deepcopy(discrete_solution), copy.deepcopy(server),
                                                             tokenizer,
                                                             DEVICE, copy.deepcopy(client_grads),
                                                             copy.deepcopy(token_embedding), dummy_label.clone().detach(),
                                                             alpha=0.01, beta=0,
        ...
        server, continuous_obj, continuous_solution, dummy_label = continuous_optimizer.optimize()
        ...
        continuous_obj = calculate_discrete_obj_label(copy.deepcopy(continuous_solution), dummy_label.clone().detach(),
        ...
        discrete_optimizer_separate = BeamSearchOptimizerIndividualLabelLongest(copy.deepcopy(server), tokenizer, DEVICE,
                                                                    sequence_length,
                                                                    BATCH_SIZE, copy.deepcopy(client_grads),
                                                                    RESULT_FILE,
                                                                    dummy_label.clone().detach(), [], longest_length,
                                                                    individual_lengths,
...
#     if len(finished) == 3:
#         stop_command = "runpodctl stop pod $RUNPOD_POD_ID"
