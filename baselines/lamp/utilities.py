...
def compute_grads(model, x_embeds, y_labels, create_graph=False):
    ...
    # outs2 = model(inputs_embeds=x_embeds, labels=y_labels)
    # print(outs.loss.item())
    # print(outs2.loss.item())
    # print("Loss are equal: ", outs.loss.item() == outs2.loss.item())
    # Only pass in the parameters that require gradients after freezing the layers
    model_parameters = []
    for param in model.parameters():
        if param.requires_grad:
            model_parameters.append(param)
    return torch.autograd.grad(outs.loss, model_parameters, create_graph=create_graph, allow_unused=True)
