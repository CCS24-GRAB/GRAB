...
from utilities import compute_grads, get_closest_tokens, get_reconstruction_loss, get_perplexity, fix_special_tokens, \
    remove_padding
from data_utils import TextDataset, TextDatasetFixed
...
def reconstruct(args, device, sample, metric, tokenizer, lm, model):
    ...
    # Turn on dropout to compute the client gradients
    model.train()
    ...
    # Turn off dropout from now on for server model so that it's more stable
    model.eval()
...
def main():
    ...
    dataset = TextDatasetFixed(args.device, args.dataset, args.split, args.n_inputs, args.batch_size)

    lm = load_gpt2_from_dict("transformer_wikitext-103.pth", device, output_hidden_states=True).to(device)
    lm.eval()

    model = AutoModelForSequenceClassification.from_pretrained(args.bert_path,
    ...
    for i in range(args.n_inputs):
        ...
        # sequences = sample[0]
        # labels = sample[1].tolist()
        # with open(f"{args.dataset}_data.txt", "a") as f:
        #     f.write(f"{sequences[0]}\n")
        #     f.write(f"{labels[0][0]}\n")
        # continue
        ...
        if args.baseline:
            result_file = f"results/dropout/{args.dropout}/{args.loss}_hard/{args.dataset}_{args.run}_run_{args.batch_size}_results.txt"
        else:
            if args.loss == "tag":
                result_file = f"results/dropout/{args.dropout}/lamp_l1l2_hard/{args.dataset}_{args.run}_run_{args.batch_size}_results.txt"
            else:
                result_file = f"results/dropout/{args.dropout}/lamp_{args.loss}_hard/{args.dataset}_{args.run}_run_{args.batch_size}_results.txt"
